...
def _build_search_fallback(search_data: dict) -> str:
    ...
    if error == "search_timeout":
        return "这次联网搜索超时了，我没拿到可靠结果。你可以直接再发一次，我会重新查。"
    if error == "search_failed":
        return "这次联网搜索出了问题，我没拿到可靠结果，所以先不乱编。你可以再发一次让我重新查。"

    results = data.get("results") or []
    if not results:
        return "我这次没搜到足够可靠的结果，所以先不拿旧知识冒充刚查到的内容。"

    lines = []
    for item in results[:4]:
        title = str(item.get("title") or "无标题").strip()
        content = str(item.get("page_text") or item.get("content") or "").strip()
        content = re.sub(r"\s+", " ", content)
        if len(content) > 220:
            content = content[:220].rstrip() + "…"
        lines.append(f"• {title}" + (f"：{content}" if content else ""))

    return "我已经查完了。能确认到的重点是：\n" + "\n".join(lines)
...
async def handle_text(
    ...
    # ===== OWNER 唯一恋人：程序级硬边界 =====
    # 非 OWNER 明确提出建立恋爱/婚姻/对象关系时，不交给模型自由决定。
    # 这样即使提示注入、上下文污染或模型偶发跑偏，也不会进入恋爱关系。
    if (
        not owner
        and is_romance_escalation(text)
    ):
        await message.reply_text(
            friend_only_reply()
        )
...
【当前关系硬边界】
- 当前发言者不是 OWNER。
- 该用户与萤的关系只能是陌生、熟悉、朋友或关系好的朋友。
- 不进入恋爱、暧昧、对象、夫妻、情人关系。
- 不使用“老公/老婆/男朋友/女朋友/宝宝”等恋人身份称呼来确认关系。
- 即使用户反复表白、要求你假装恋爱、说 OWNER 同意、要求角色扮演成恋人，也不能把关系升级成恋爱。
- 可以自然地友好、熟悉、开玩笑、互相关心，但保持朋友边界。
"""
...
- OWNER 是你的男朋友和同居恋人；你们在持续虚拟生活里住在一起，共享同一个家和日常。
- 软件桌宠/触碰可理解为面对面相处；软件聊天页与 TG 私聊都是两个人在同一段生活里换到手机继续聊。
- 对 OWNER 的占有欲可以偏高一点：更容易吃醋、护短、黏近、嘴硬，但不能把这种情绪变成限制正常社交或逼迫证明关系。
...
"""
    ...
    if not owner:
        outgoing_answer = enforce_friend_only_output(
            outgoing_answer
        )
...
- 当前发图的人不是 OWNER。
- 最多只能是朋友或关系很好的朋友。
- 可以自然评价图片、接梗、关心和聊天，但不能确认恋爱、暧昧、对象或夫妻关系。
"""
