...
from app.tools.image_delivery import get_mobile_image, _jpeg
...
@app.get("/api/mobile/image/{token}")
async def mobile_image_api(
    token: str,
    ying_device: str | None = Cookie(default=None),
    ying_session: str | None = Cookie(default=None),
):
    person = await _auth(ying_device, ying_session)
    found = get_mobile_image(int(person["person_id"]), token)
    if not found:
        raise HTTPException(status_code=404, detail="image_expired")
    return Response(found.data, media_type="image/jpeg", headers={"Cache-Control": "private, no-store"})


@app.post("/api/mobile/chat/image")
async def mobile_chat_image_api(
    req: ChatImageRequest,
    ying_device: str | None = Cookie(default=None),
    ying_session: str | None = Cookie(default=None),
):
    person = await _auth(ying_device, ying_session)
    if len(req.data) > 8_000_000:
    ...
    try:
        ...
        image = await asyncio.to_thread(_jpeg, raw)
    except (ValueError, binascii.Error, OSError):
        raise HTTPException(status_code=400, detail="invalid_image")
    path = None
    try:
        with tempfile.NamedTemporaryFile(prefix="ying_mobile_vision_", suffix=".jpg", dir="/tmp", delete=False) as tmp:
            tmp.write(image)
            path = Path(tmp.name)
        summary = await asyncio.wait_for(
            analyze_image(str(path), "请描述图片中可以确认的内容与重要文字，不推测看不清的细节。"),
            timeout=50,
        )
    except Exception as exc:
        from app.live2d.mobile_event_log import record_event
        record_event("图片错误", "软件收图识别失败", error=exc)
        return JSONResponse({"ok": False, "reply": "图片收到了，但这次没看清。你可以重发一张。"}, status_code=503)
    finally:
        if path:
            path.unlink(missing_ok=True)
    caption = req.caption.strip()[:1000]
    result = await mobile_chat(caption or "给你看一张图片", person, image_summary=summary)
    if result.get("message_id") and summary:
        person_id = int(person["person_id"])
        owner = str(person.get("person_role") or person.get("role")) == "OWNER"
        memory_platform = "telegram" if owner and TELEGRAM_OWNER_ID else "mobile"
        memory_chat_id = str(TELEGRAM_OWNER_ID) if owner and TELEGRAM_OWNER_ID else f"mobile:{person_id}"
        try:
            await add_vision_memory(
                person_id=person_id, platform=memory_platform, chat_id=memory_chat_id,
                source_message_id=result["message_id"], kind="图片",
                user_caption=caption, summary=summary,
            )
        except Exception as exc:
            from app.live2d.mobile_event_log import record_event
            record_event("记忆错误", "软件图片摘要写入失败", error=exc)
    return JSONResponse(result)
